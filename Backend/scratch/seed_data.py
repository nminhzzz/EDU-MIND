...
async def seed_data():
    ...
    try:
        ...
        classrooms_data = [
            ("CLASS_TRIETHOC", "Lớp Triết học Cơ bản", "Môn Triết học đại cương cho tân sinh viên", "TRIETHOC"),
            ("CLASS_JAVA", "Lớp Lập trình Java OOP", "Hướng đối tượng Java cơ bản đến nâng cao", "JAVA"),
            ("CLASS_CSHARP", "Lớp Lập trình C# chuyên sâu", "Lập trình C# / .NET / LINQ", "CSHARP")
        ]
        ...
        for subj_code, chunks in MATERIALS_DATA.items():
            subj = subjects_map[subj_code]
            for chunk in chunks:
                print(f"  -> Đang embed chủ đề '{chunk['topic']}' của môn {subj_code}...")
                try:
                    inserted_id = await save_study_material(
                        ...
                        metadata={"source": "Giáo trình chính thống", "author": "Hệ thống"}
                    )
