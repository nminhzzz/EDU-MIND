...
from app.services.recommendation_service import get_pending_reviews, review_recommendation
...
def run_teacher_flow_test():
    ...
    try:
        ...
        print(f"  -> Tạo lớp học thành công! ID: {classroom.id}, Code: {classroom.class_code}")

        # 3. Gán môn học và học sinh vào lớp bằng Service
        print("\nStep 3: Gán môn học và học sinh vào lớp...")
        add_subject_to_classroom(db=db, classroom_id=classroom.id, teacher_id=teacher.id, subject_id=subj.id)
        add_student_to_classroom(db=db, classroom_id=classroom.id, teacher_id=teacher.id, student_email=student.email)
        ...
        db.query(AIRecommendationReview).filter(AIRecommendationReview.student_id == student.id).delete()
        db.commit()
        ...
        for p in plans:
            print(f"     Nhiệm vụ: {p.title} (Ngày: {p.study_date}, Thời gian: {p.start_time}-{p.end_time})")
