...
async def test_chat_deletion_async():
    ...
    messages_count = await db["tutor_messages"].count_documents({"session_id": ObjectId(session_id)})
    print(f"MongoDB verification before deletion: Session exists={session_doc is not None}, Messages count={messages_count}")
    assert messages_count > 0
    
    # 4. Gửi yêu cầu DELETE xóa session
    print("\n--- 3. DELETING THE SESSION VIA API ---")
    delete_res = requests.delete(
        f"{BASE_URL}/api/v1/chat/tutor/session/{session_id}",
        headers=headers
    )
    ...
    session_doc_after = await db["tutor_sessions"].find_one({"_id": ObjectId(session_id)})
    messages_count_after = await db["tutor_messages"].count_documents({"session_id": ObjectId(session_id)})
    print(f"MongoDB verification after deletion: Session exists={session_doc_after is not None}, Messages count={messages_count_after}")
