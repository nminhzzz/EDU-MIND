...
async def run_interactive_test():
    ...
    try:
        ...
        student = db.query(User).filter(User.email == "student_interactive@test.com").first()
        if not student:
            student = User(
        ...
        subject = db.query(Subject).filter(Subject.code == "TRIETHOC_INTERACTIVE").first()
        if not subject:
            subject = Subject(
        ...
        db_plans = db.query(StudyPlan).filter(StudyPlan.goal_id == confirm_result['goal'].id).order_by(StudyPlan.study_date.asc()).all()
