...
def run_cookie_auth_test():
    ...
    print(
        f"  - access_token cookie: {'Hoạt động' if access_token_cookie else 'Không tìm thấy'}"
    )
    print(
        f"  - refresh_token cookie (HttpOnly): {'Hoạt động' if refresh_token_cookie else 'Không tìm thấy'}"
    )
    ...
    ), f"Không thể lấy thông tin /me bằng cookie: {me_res.text}"
    print("  -> Lấy thông tin cá nhân qua Cookie thành công!")
    print(
        f"  -> Người dùng: {me_res.json()['full_name']} | Email: {me_res.json()['email']}"
    )
    ...
    ), "Người dùng vẫn đăng nhập sau khi logout!"
