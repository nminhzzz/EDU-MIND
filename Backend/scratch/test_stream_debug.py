...
async def test():
    ...
    from app.agents.roadmap_planner import generate_unified_plan_stream
    ...
    async for msg_type, msg_data in generate_unified_plan_stream(
        ...
        if msg_type == "token":
            ...
            print(
                f"Stream complete: weeks={len(msg_data.weeks)} daily={len(msg_data.daily_schedule)}"
            )
        elif msg_type == "error":
