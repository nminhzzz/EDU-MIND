...
def test_quiz_completion_logic():
    ...
    if not plan:
        print(
            "No study plan found, please run test_confirm_api.py first to create a roadmap."
        )
        return
    ...
    patch_res = requests.patch(
        f"{BASE_URL}/api/v1/plans/{plan_id}", json={"status": "done"}, headers=headers
    )
    ...
    patch_doing_res = requests.patch(
        f"{BASE_URL}/api/v1/plans/{plan_id}", json={"status": "doing"}, headers=headers
    )
    ...
    submit_low_res = requests.post(
        ...
        json={"answers": answers_low, "duration_seconds": 120},
        headers=headers,
    )
    print(f"Submit low score status code: {submit_low_res.status_code}")
    print(
        f"Attempt result: Score {submit_low_res.json()['score']}/10.0, Correct: {submit_low_res.json()['correct_count']}"
    )
    ...
    progress_low = (
        ...
        .filter(StudyPlanProgress.study_plan_id == plan_id)
        .first()
    )
    print(
        f"StudyPlan Status after low score: '{plan_after_low.status}' (Expected: 'doing')"
    )
    print(
        f"Completion Percent after low score: {float(progress_low.completion_percent) if progress_low else 0.0}% (Expected: 50.0%)"
    )
    ...
    submit_high_res = requests.post(
        f"{BASE_URL}/api/v1/quizzes/{quiz_id}/submit",
        json={"answers": answers_high, "duration_seconds": 150},
        headers=headers,
    )
    print(f"Submit high score status code: {submit_high_res.status_code}")
    print(
        f"Attempt result: Score {submit_high_res.json()['score']}/10.0, Correct: {submit_high_res.json()['correct_count']}"
    )
    ...
    print(
        f"StudyPlan Status after high score: '{plan_after_high.status}' (Expected: 'done')"
    )
    print(
        f"Completion Percent after high score: {float(progress_high.completion_percent) if progress_high else 0.0}% (Expected: 100.0%)"
    )

    assert plan_after_high.status == "done", "Task should automatically become 'done'!"
    assert (
        float(progress_high.completion_percent) == 100.0
