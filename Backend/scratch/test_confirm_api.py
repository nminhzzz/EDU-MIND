...
def test_confirm():
    ...
    try:
        ...
        plans = db.query(StudyPlan).filter(
            ...
            StudyPlan.rag_content != None
        ).all()
        print(f"Found {len(plans)} study plan tasks with attached RAG curriculum materials!")
        for p in plans[:2]:
        ...
        quizzes = db.query(Quiz).filter(
            ...
            Quiz.study_plan_id != None
        ).all()
        print(f"Found {len(quizzes)} quizzes successfully linked to daily study plan tasks!")
        for q in quizzes[:2]:
