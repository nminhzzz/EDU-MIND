...
def run_integration_test():
    ...
    res = requests.post(f"{BASE_URL}/auth/login", json={"email": "teacher@edumind.com", "password": "teacherpassword"})
    ...
    res = requests.post(f"{BASE_URL}/classrooms/", json=class_payload, headers=t_headers)
    ...
    add_student_payload = {
        "student_email": "student_test@edumind.com"
    }
    res = requests.post(f"{BASE_URL}/classrooms/{classroom_id}/students", json=add_student_payload, headers=t_headers)
    ...
    res = requests.get(f"{BASE_URL}/classrooms/{classroom_id}/students/progress", headers=t_headers)
    ...
    files = {
        "file": ("on_tap_toan.txt", "Noi dung tai lieu on tap toan dai so...", "text/plain")
    }
    data = {
        "subject_id": subject_id,
        "title": "Tài liệu ôn tập Toán Đại số chương 1"
    }
    res = requests.post(f"{BASE_URL}/documents/", data=data, files=files, headers=t_headers)
    ...
    res = requests.post(f"{BASE_URL}/quizzes/teacher/create", json=quiz_payload, headers=t_headers)
    ...
    res = requests.post(f"{BASE_URL}/auth/login", json={"email": "student_test@edumind.com", "password": "studentpassword"})
    ...
    submit_payload = {
        "answers": [
            {"question_index": 0, "answer": "B", "is_correct": True}
        ],
        "duration_seconds": 45
    }
    res = requests.post(f"{BASE_URL}/quizzes/{quiz_id}/submit", json=submit_payload, headers=s_headers)
    ...
    res = requests.get(f"{BASE_URL}/quizzes/classroom/{classroom_id}/attempts", headers=t_headers)
