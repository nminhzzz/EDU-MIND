...
async def run_chat_tutor_memory_test():
    print(
        "--- KHỞI CHẠY KIỂM THỬ TÍCH HỢP TRÍ NHỚ & TÓM TẮT HỘI THOẠI (CHAT TUTOR) ---"
    )
    ...
    try:
        ...
        student = (
            db.query(User).filter(User.email == "student_tutor_test@test.com").first()
        )
        if not student:
            student = User(
        ...
        assert (
            "hegel" in reply_2.lower()
            or "biện chứng" in reply_2.lower()
            or "phép biện chứng" in reply_2.lower()
            or "tinh thần" in reply_2.lower()
        ...
        for idx, msg in enumerate(test_chats):
            ...
            reply, messages = await tutor_agent.chat_with_tutor(
                user_message=msg, student_id=student.id, session_id=session_id
            )
        ...
        session_doc = await mongo_db["tutor_sessions"].find_one(
        ...
        msg_count = await mongo_db["tutor_messages"].count_documents(
            {"session_id": ObjectId(session_id)}
        )
        print(
            f"-> Số lượng tin nhắn chi tiết còn lại trong MongoDB: {msg_count} (Ngưỡng MAX_MESSAGES = 10)"
        )
        ...
        assert (
            "hegel" in reply_final.lower()
            or "marx" in reply_final.lower()
            or "triết" in reply_final.lower()
