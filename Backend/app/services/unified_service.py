...
from app.agents.roadmap_planner import (
    generate_unified_plan,
    generate_unified_plan_stream,
)
...
def prune_roadmap_history(history: List[Dict[str, str]]) -> List[Dict[str, str]]:
    ...
    3. Tin nhắn hiện tại của User (báo nghỉ bận).
    """
    ...
    if last_user_msg and last_user_msg != first_msg:
        pruned.append(last_user_msg)
...
async def generate_unified_draft(
    ...
    pref = (
        db_mysql.query(StudentPreference)
    ...
    if pref:
        study_hours_per_day = (
            float(pref.study_hours_per_day) if pref.study_hours_per_day else 2.0
        )
        ...
        if available_schedule:
            ...
            for k, v in available_schedule.items():
                is_available = (
                    any(v.get(slot) for slot in ["morning", "afternoon", "evening"])
...
async def confirm_unified_draft(
    ...
    if not plan:
        print(
            f"-> Redis Cache MISS or expired for key {redis_key}. Fallback to MongoDB."
        )
    ...
    # 0. Hủy bỏ tất cả các lộ trình (StudyGoal) cũ đang hoạt động của môn học này cho học sinh này
    db.query(StudyGoal).filter(
        StudyGoal.student_id == student.id,
        StudyGoal.subject_id == subject_obj.id,
        StudyGoal.status == "active",
    ).update({StudyGoal.status: "cancelled"}, synchronize_session=False)
    ...
    for day_task in plan.daily_schedule:
        # Bỏ qua các ngày trống không có bài học (ví dụ ngày nghỉ được sinh trống)
        ...
        try:
            ...
            continue  # Nếu ngày học sai định dạng thì bỏ qua

        # Fallback giờ học hợp lệ nếu trống hoặc sai định dạng
    ...
    for quiz_item in plan.quizzes:
        ...
        for p in db_plans:
            ...
            if len(clean_quiz_title) >= 3 and (
                clean_quiz_title in p.title.lower()
                or p.title.lower() in clean_quiz_title
            ):
...
async def generate_materials_and_quizzes_for_plans_bg(
    ...
    try:
        ...
        # Lấy thông tin môn học để tùy chỉnh cấu trúc bài giảng RAG cho phù hợp
        from app.models.subject import Subject

        subject_obj = db.query(Subject).filter(Subject.id == subject_id).first()
        subject_name = subject_obj.name.lower() if subject_obj else ""

        # Kiểm tra xem có phải môn học nặng lý thuyết khoa học xã hội không
        is_theory_subject = any(
            keyword in subject_name
            for keyword in [
                "triết học",
                "pháp luật",
                "lý luận",
        ...
        # Chỉ sinh trước cho 5 ngày học đầu tiên để tránh nghẽn server và quá tải API
        # Các ngày học sau sẽ được sinh on-the-fly (on demand) khi học sinh bắt đầu học
        plans = sorted(plans, key=lambda x: x.study_date)
        plans_to_generate = plans[:5]

        print(
            f"[BG Task] Bắt đầu sinh ngầm tài liệu & quiz cho lộ trình {goal_id}. Sinh trước 5 ngày học đầu tiên / {len(plans)} tổng số ngày."
        )

        for p in plans_to_generate:
            ...
            if materials:
                ...
                # Tùy chỉnh instruction dựa theo loại môn học
                ...
                try:
                    # Chạy đồng thời qua thread pool để tránh khóa event loop chính của FastAPI
                    ...
                    db.commit()
                    print(
                        f"[BG Task] Đã sinh tài liệu RAG chi tiết cho bài học {p.id}: {p.title}"
                    )
                except Exception as e:
            ...
            if not existing_quiz:
                try:
                    ...
                    print(
                        f"[BG Task] Đã sinh và liên kết đề thi 10 câu thành công cho bài học {p.id}"
                    )
                except Exception as e:
                    print(f"[BG Task] Lỗi sinh quiz cho bài học {p.id}: {e}")

            # Trả quyền kiểm soát lại cho event loop sau mỗi ngày học để các request khác không bị block
