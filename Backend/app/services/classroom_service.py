...
def get_classroom_students_progress(
    ...
    if current_user_role != "admin" and classroom.teacher_id != current_teacher_id:
        raise HTTPException(status_code=403, detail="Bạn không có quyền quản lý lớp học này.")

    enrollments = db.query(ClassroomStudent).filter(ClassroomStudent.classroom_id == classroom_id).all()
    ...
    for en in enrollments:
        ...
        total_goals = db.query(StudyGoal).filter(StudyGoal.student_id == student.id).count()
        completed_goals = db.query(StudyGoal).filter(
            StudyGoal.student_id == student.id, StudyGoal.status == "completed"
        ).count()
        total_attempts = db.query(QuizAttempt).filter(QuizAttempt.student_id == student.id).count()
        
        avg_score = db.query(func.avg(QuizAttempt.score)).filter(QuizAttempt.student_id == student.id).scalar()
...
def remove_student_from_classroom(
    ...
    enrollment = classroom_student_repository.get_by_relation(db, classroom_id, student_id)
    if not enrollment:
        raise HTTPException(status_code=404, detail="Học sinh không tham gia lớp học này.")

    db.delete(enrollment)
    db.commit()


def list_all_classrooms_admin(db: Session, skip: int = 0, limit: int = 100) -> List[Classroom]:
