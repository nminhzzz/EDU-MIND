...
async def upload_document(
    ...
    try:
        ...
        # Chỉ decode văn bản nếu là định dạng text cơ bản
        if file_type in ["txt", "md", "json", "csv", "html"]:
            content = file_bytes.decode("utf-8", errors="ignore")
        else:
            # Fallback mô tả cho tài liệu dạng nhị phân (PDF/DOCX)
            content = f"Tài liệu giảng dạy môn {subject.name}: {title}. Định dạng file: {file_type}."

        db_mongo = await get_mongodb_db()
...
async def delete_document(
    ...
    try:
        ...
        if db_mongo is not None:
            ...
            await db_mongo.study_material_embeddings.delete_many({"metadata.document_id": document_id})
    except Exception as e:
