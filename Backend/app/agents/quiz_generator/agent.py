...
"""
    ...
    for attempt in range(attempts):
        try:
            ...
            if len(quiz_res.questions) >= total_questions:
                quiz_res.questions = quiz_res.questions[:total_questions]
                return quiz_res
            else:
                raise ValueError(
                    f"LLM chỉ sinh được {len(quiz_res.questions)}/{total_questions} câu hỏi không trùng lặp."
                )

        except Exception as e:
            last_error = e
            print(f"[Warning] Quiz Generator Agent: Lượt thử {attempt+1} thất bại: {e}")
...
"""
    ...
    for attempt in range(attempts):
        try:
            ...
            last_error = e
            print(
                f"[Warning] Correct Quiz Questions: Lượt thử {attempt+1} thất bại: {e}"
            )
