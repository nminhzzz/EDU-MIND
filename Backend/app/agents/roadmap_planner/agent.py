...
async def generate_unified_plan(
    ...
    context_str = ""
    print("-> roadmap_planner: Starting RAG vector search in MongoDB...")
    ...
    analytics_str = ""
    print("-> roadmap_planner: Querying MySQL for student analytics...")
    try:
...
1. Thiết lập Lộ trình tuần (weeks) và phân chia Lịch học chi tiết từng ngày (daily_schedule) bắt đầu từ {current_date}.
   ...
   - ĐẶC BIỆT CHÚ Ý YÊU CẦU TINH CHỈNH: Xem kỹ tin nhắn phản hồi của học sinh. Nếu học sinh báo bận vào một ngày cụ thể (ví dụ: ngày mai, ngày kia, hoặc một ngày cụ thể nào đó), bạn TUYỆT ĐỐI không được xếp lịch học (`daily_schedule`) vào ngày đó.
...
Chỉ trả về JSON thuần túy, không kèm markdown, không kèm lời dẫn."""
    ...
    print("-> roadmap_planner: Calling generate_content_nvidia...")
    ...
    print(
        f"-> roadmap_planner: generate_content_nvidia returned response of size {len(response_text)}"
    )

    try:
        # Hậu xử lý trích xuất JSON nếu AI trả kèm lời thoại bên ngoài
        ...
        # Hậu xử lý sửa ngày học thực tế theo lịch rảnh học sinh (tránh lỗi LLM lập lệch ngày bận)
        if "daily_schedule" in data and isinstance(data["daily_schedule"], list):
            # Lọc bỏ các bài học rỗng/trống không có tiêu đề (do LLM sinh trống ngày nghỉ)
            filtered_schedule = [
                item
            ...
            while len(available_dates) < needed_days:
                ...
                if weekday_key not in off_days and date_str not in manually_busy_dates:
