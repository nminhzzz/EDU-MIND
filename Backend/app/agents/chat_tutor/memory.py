...
async def create_tutor_session(student_id: int, subject_id: int, title: str = "Trò chuyện cùng Gia sư ảo") -> str:
    """
...
async def get_tutor_sessions(student_id: int, subject_id: Optional[int] = None) -> List[Dict]:
    """
...
async def summarize_session_if_needed(session_id: str) -> None:
    ...
    msg_count = await db["tutor_messages"].count_documents({"session_id": ObjectId(session_id)})
    ...
    cursor = db["tutor_messages"].find(
        {"session_id": ObjectId(session_id)}
    ).sort("created_at", 1).limit(MESSAGES_TO_SUMMARIZE)
    ...
    prompt = (
        "Bạn là một trợ lý ảo phân tích hội thoại. Nhiệm vụ của bạn là cập nhật tóm tắt lịch sử cuộc trò chuyện giữa Học sinh (user) và Gia sư ảo (assistant/model).\n\n"
    )
    if current_summary:
        prompt += f"Tóm tắt bối cảnh cũ trước đó:\n{current_summary}\n\n"
        prompt += f"Các tin nhắn hội thoại mới diễn ra tiếp theo:\n{conversation_text}\n\n"
    ...
    try:
        ...
        print(f"-> Summarized {MESSAGES_TO_SUMMARIZE} messages for tutor session {session_id}. New summary size: {len(new_summary)} chars.")
        
        # 6. Xóa các tin nhắn đã tóm tắt khỏi collection tutor_messages
        await db["tutor_messages"].delete_many(
