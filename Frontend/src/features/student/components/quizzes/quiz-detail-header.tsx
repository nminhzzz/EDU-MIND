...
export function QuizDetailHeader({
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 border border-zinc-200/80 dark:border-zinc-800 p-6 rounded-md shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div>
        <span className="text-[10px] font-bold text-indigo-650 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/40 px-2.5 py-1 rounded-md uppercase tracking-wide">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {!isReview && tabViolations > 0 && (
          <div
            className={`flex items-center gap-1.5 px-3.5 py-2 rounded-md text-xs font-bold transition-all ${
              tabViolations >= 2
        ...
        {isReview ? (
          <div className="flex items-center gap-2 text-emerald-600 bg-emerald-50 dark:bg-emerald-950/40 px-4 py-2 rounded-md text-xs font-bold border border-emerald-100 dark:border-emerald-900/40">
            <Trophy className="w-4 h-4" />
            Đã hoàn thành luyện tập
          </div>
        ) : (
          <div
            className={`flex items-center gap-2 px-4 py-2 rounded-md text-xs font-black transition-all border ${
              isTimeCritical
