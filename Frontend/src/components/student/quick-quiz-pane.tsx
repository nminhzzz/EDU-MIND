...
interface QuickQuizPaneProps {
  ...
  topic: string;
  studyPlanId: number;
  onSuccess?: () => void; // Triggered when the student completes the quiz
}
...
export function QuickQuizPane({ subjectId, topic, studyPlanId, onSuccess }: QuickQuizPaneProps) {
  ...
  useEffect(() => {
    ...
    const fetchExistingQuiz = async () => {
      try {
        const res = await apiClient.get<Quiz>(`/quizzes/plan/${studyPlanId}`);
        if (active) {
          setQuiz(res.data);
  ...
  const handleGenerate = async () => {
    setLoading(true);
    try {
      const res = await apiClient.post<Quiz>("/quizzes/generate", {
      ...
      toast.error(err?.response?.data?.detail || "Không thể khởi tạo đề kiểm tra nhanh.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await apiClient.post<AttemptResult>(`/quizzes/${quiz.id}/submit`, payload);
      setResult(res.data);
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-zinc-400 gap-2">
        <Loader2 className="w-6 h-6 animate-spin text-indigo-600" />
        <span className="text-xs font-semibold">AI đang đọc hiểu giáo trình và sinh câu hỏi trắc nghiệm RAG...</span>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center text-center p-6 border border-zinc-200 dark:border-zinc-800 rounded-xl bg-zinc-50 dark:bg-zinc-950/20">
        ...
        <p className="text-[10px] text-zinc-400 dark:text-zinc-500 mt-1 max-w-xs leading-relaxed font-semibold">
          AI sẽ sinh 10 câu hỏi trắc nghiệm bám sát theo giáo trình môn học và chủ đề này để bạn kiểm tra nhanh kết quả nghiên cứu.
        </p>
        <button
          onClick={handleGenerate}
          className="mt-4 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-extrabold rounded-lg transition-transform active:scale-[0.98] cursor-pointer shadow-md shadow-indigo-500/10"
        >
